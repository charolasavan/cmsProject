...
//             if (response.data) {
//                 setCartData(response.data.cart_items || response.data);
//                 setCouponCode(response.data.coupon_code || '');
//                 setCartSummary({
//                     sub_total_price: response.data.sub_total_price || 0,
...
//             formData.append('cart_items', JSON.stringify(updateData));

//             const response = await api.put(`/addtocart/update`, formData);
...
function AddToCart() {
    ...
    // Calculate subtotal and total based on cartData and discount
    const calculateCartSummary = (items, discount = 0) => {
        const subTotal = items.reduce((acc, item) => {
            const price = item.selling_price || item.regular_price;
            return acc + price * item.product_quantity;
        }, 0);
        setCartSummary({
    ...
    const handleQuantityChange = (e, cart_id) => {
        ...
        calculateCartSummary(updatedCart, cartSummary.discount); // update total dynamically
    };
    ...
    const handleUpdateCart = async () => {
        try {
            ...
            if (response.data) {
                // Update table and summary from response
